...
describe('LoanFlow portfolio journeys', () => {
  ...
  it('keeps the draft after a simulated service failure', () => {
    ...
    cy.get('[data-cy="continue"]').click();
    cy.get('#firstName').should('have.value', 'Kamilla');
  });

  it('renders the component showcase', () => {
    cy.visit('/components');
    cy.contains('Component showcase').should('be.visible');
